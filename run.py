...
parser.add_argument('--train_data_paths', type=str, default='../moving-mnist-train.npz')
parser.add_argument('--valid_data_paths', type=str, default='../moving-mnist-valid.npz')
...
def schedule_sampling(eta, itr):
    ...
    random_flip = np.random.random_sample(
        (args.batch_size, args.total_length - args.input_length - 1))   # (8,9)
    true_token = (random_flip < eta)
    ones = np.ones((args.img_width // args.patch_size,       # (16,16,16)
                    args.img_width // args.patch_size,
                    args.patch_size ** 2 * args.img_channel))
...
def train_wrapper(model, writer):
    ...
    for itr in range(1, args.max_iterations + 1):
        if train_input_handle.no_batch_left():
            train_input_handle.begin(do_shuffle=True)   # current itr ends, shuffle the data and begin another itr
        ims = train_input_handle.get_batch()
        # print('size', ims.shape)
        ims = preprocess.reshape_patch(ims, args.patch_size)
        # print('ims.type',ims.type())
